...
| `outcome`       | `Enum(ProcessingOutcome)`             | `PROCESSED`, `FAILED`, `SKIPPED_NOT_INVOICE`, `SKIPPED_DUPLICATE`, `RETRYING` |
...
(new email discovered)
   ...
   (email's SourceEmail already has an Invoice) ──► SKIPPED_DUPLICATE (attemptNumber += 1,
                                                     re-linked to the existing invoiceId)
```
...
visible, not just the final outcome. Re-evaluating an already-processed email (idempotent re-runs,
Principle II) never creates a second `Invoice`/`Attachment` set; it records a `SKIPPED_DUPLICATE`
entry linked to the original `invoiceId` instead.
...
- `ProcessingOutcome`: `PROCESSED` \| `FAILED` \| `SKIPPED_NOT_INVOICE` \| `SKIPPED_DUPLICATE` \|
  `RETRYING`
